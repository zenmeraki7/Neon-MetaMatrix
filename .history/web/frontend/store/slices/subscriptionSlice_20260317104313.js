...
export const verifyActivePlan = createAsyncThunk(
  "subscription/verifyPlan",
  async (_, { rejectWithValue }) => {
    try {
      const response = await subscriptionService.verifyPlan();
      return response;
    } catch (error) {
      return rejectWithValue(error.message || "Failed to verify active plan");
    }
  }
);
